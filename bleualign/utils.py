...
def evaluate(options, testalign, goldalign, log_function):
    ...
    for pair in [(len(srclist), len(targetlist)) for srclist, targetlist in goldalign]:
        ...
        for pair in paircounts:
            pairs_normalized[pair] = (
                paircounts[pair], paircounts[pair] / float(len(goldalign)))
    ...
    (tpstrict, fnstrict, tplax, fnlax) = recall(
        (0, 0), goldalign, [i[0] for i in testalign], log_function)
    ...
    for aligntype in set([i[1] for i in testalign]):
        ...
        log_function('precision for alignment type ' + str(aligntype) +
                     ' ( ' + str(len(testalign_bytype)) + ' alignment pairs)')
        precision(goldalign, testalign_bytype, log_function)

    log_function('\ntotal precision:', end='')
    log_function(str(len(testalign)) + ' alignment pairs found')
    (tpstrict, fpstrict, tplax, fplax) = precision(
        goldalign, testalign, log_function)
    ...
    for (src, target) in [i[0] for i in testalign]:
        ...
        if (src, target) in goldalign:
            ...
            for srclist, targetlist in goldalign:
                # lax condition: hypothesis and gold alignment only need to
                # overlap
...
def recall(aligntype, goldalign, testalign, log_function):
    ...
    if srclen == 0 and targetlen == 0:
        ...
        gapdists = [(len(srclist), len(targetlist))
                    for srclist, targetlist in goldalign]
...
def finalevaluation(results, log_function):
    ...
    try:
        pstrict = (
            precision_value[0] / float(precision_value[0] + precision_value[1]))
    except ZeroDivisionError:
        pstrict = 0
    try:
        plax = (
            precision_value[2] / float(precision_value[2] + precision_value[3]))
    except ZeroDivisionError:
        plax = 0
    try:
