...
def cook_test(test, args, n=4):
    ...
    for (ngram, count) in counts.items():
        result["correct"][
            len(ngram) - 1] += min(refmaxcounts.get(ngram, 0), count)
...
def score_cooked(allcomps, n=4):
    totalcomps = {
        'testlen': 0, 'reflen': 0, 'guess': [0] * n, 'correct': [0] * n}
    ...
    for k in range(n):
        ...
        logbleu += math.log(totalcomps['correct']
                            [k]) - math.log(totalcomps['guess'][k])
    logbleu /= float(n)
