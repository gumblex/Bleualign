...
from bleualign.nltkbleu import doublebleu
    ...
    while not EOF:
        ...
        for text, translations in ((src, srctotarget), (target, targettosrc)):
            ...
            for line in text:
                ...
                if options['factored']:
                    rawline = ' '.join(
                        word.split('|')[0] for word in line.split())
...
# best call this in a separate process because we limit the queue size for
# memory reasons
def tasks_producer(tasks, num_tasks, data):
    ...
    for i in xrange(number_of_threads):
        tasks.put((None, None))
    # only if this point is reached, process finishes when all tasks are done.
    num_tasks.value -= 1
...
class Aligner:
    default_options = {
        ...
        # False for normal text, True for 'text | other information', seprating
        # by '|'
        'factored': False,
        ...
        # only consider target sentences for bleu-based alignment that are
        # among top N alternatives for a given source sentence
        'maxalternatives': 3,

        # bleu scoring algorithm works with 4-grams by default. We got better
        # results when using 2-grams (since there are less 0 scores then)
        'bleu_ngrams': 2,
        ...
        # must be reliable (article i in the source text needs to correspond to
        # article i in the target text)
        ...
        # set filterlang True, whose when you want to filter alignemts which
        # src is similar to target than translation
        ...
        # if not passing anything or assigning None, they will use StringIO to
        # save results.
        'output': None,
    ...
    def mainloop(self):
        ...
        if multiprocessing_enabled:
            ...
            scorers = [AlignMultiprocessed(
                tasks, self.options, scores, self.log) for i in xrange(number_of_threads)]

            for p in scorers:
                p.start()

            # this function produces the alignment tasks for the consumers in
            # scorers
            ...
            while i < num_tasks.value:
                ...
                # no translation provided. we copy source sentences for further
                # processing
                else:
                ...
                if self.options['eval']:
                    ...
                    results[i] = evaluate(
                        self.options, self.multialign, self.options['eval'][i], self.log)
    ...
    # Start different alignment runs depending on which and how many
    # translations are sent to program; intersect results.
    def process(self, sourcelist, targetlist, translist1, translist2):
        ...
        for i, translist in enumerate(translist1):
            self.log(
                "computing alignment between srctotarget (file " + str(i) + ") and target text", 1)
            phase1.append(self.align(translist, raw_targetlist))

        for i, translist in enumerate(translist2):
            self.log(
                "computing alignment between targettosrc (file " + str(i) + ") and source text", 1)
        ...
        if len(phase1) > 1:
            ...
            phase1 = sorted(
                set(phase1[0]).intersection(*[set(x) for x in phase1[1:]]))
        elif phase1:
            phase1 = phase1[0]
        ...
        if len(phase2) > 1:
            ...
            phase2 = sorted(
                set(phase2[0]).intersection(*[set(x) for x in phase2[1:]]))
        elif phase2:
            phase2 = phase2[0]
        ...
        if phase1 and phase2:
            ...
            for pair, t in phase1:
                if pair in phase2mirror:
                    phase3.append(
                        (pair, 'INTERSECT: ' + t + ' - ' + phase2[phase2mirror.index(pair)][1]))
            multialign = phase3
    ...
    def align(self, translist, targetlist):
        ...
        if self.options["galechurch"]:
            ...
            translist = list(enumerate(translist))
            targetlist = list(enumerate(targetlist))
    ...
    def eval_sents_dummy(self, translist, targetlist):
        ...
        for testID, testSent in enumerate(translist):
            ...
            for refID, refSent in enumerate(targetlist):
                # replace this with your own similarity score
                score = 100 - abs(len(testSent) - len(refSent))
                if score > 0:
                    scores.append((score, refID, score))
            scoredict[testID] = sorted(scores, key=itemgetter(0), reverse=True)[
    ...
    def eval_sents_nltkbleu(self, translist, targetlist):
        scoredict = {}
        ngweight = [1 / self.options['bleu_ngrams']] * \
            self.options['bleu_ngrams']

        for testID, testSent in enumerate(translist):
            scores = []

            for refID, refSent in enumerate(targetlist):
                score = doublebleu(testSent, refSent, ngweight)
                # replace this with your own similarity score
                if score > 0:
                    scores.append((score, refID, score))
            scoredict[testID] = sorted(scores, key=itemgetter(0), reverse=True)[
    ...
    # follow the backpointers in score matrix to extract best path of 1-to-1
    # alignments
    ...
    def pathfinder(self, translist, targetlist):
        ...
        # add an extra row/column to the matrix and start filling it from 1,1
        # (to avoid exceptions for first row/column)
        matrix = [[0] * (len(targetlist) + 1)
                  for row in xrange(len(translist) + 1)]
        pointers = [[''] * len(targetlist) for row in xrange(len(translist))]

        for i in xrange(len(translist)):
            alignments = dict((target, score)
                              for (score, target, correct) in self.scoredict[i])

            for j in xrange(len(targetlist)):
    ...
    # gapfiller() takes two sentence pairs and all unaligned sentences in
    # between as arguments; gapfinder() extracts these.
    def gapfinder(self, translist, targetlist):
        ...
        for src, target in self.bleualign:
            ...
            # identify gap sizes
            sourcegap = list(xrange(oldsrc[-1] + 1, src))
            targetgap = list(xrange(oldtarget[-1] + 1, target))

            if targetgap or sourcegap:
                lastpair = self.gapfiller(
                    sourcegap, targetgap, lastpair, ((src,), (target,)), translist, targetlist)
            else:
        ...
        sourcegap = list(xrange(src + 1, len(translist)))
        targetgap = list(xrange(target + 1, len(targetlist)))

        if targetgap or sourcegap:
            lastpair = self.gapfiller(
                sourcegap, targetgap, lastpair, ((), ()), translist, targetlist)
    ...
    def gapfiller(self, sourcegap, targetgap, pregap, postgap, translist, targetlist):
        ...
        # compile list of sentences in gap that will be considered for BLEU
        # comparison
        if self.options['Nto1'] > 1 or "bleu1to1" in self.options['gapfillheuristics']:
            ...
            nSrc = {}
            for n in xrange(2, self.options['Nto1'] + 1):
            ...
            scoredict_raw = self.eval_sents_double(evalsrc_raw, evaltarget_raw)
        ...
        while sourcegap or targetgap:
            ...
            if sourcegap and self.options['Nto1'] > 1:
                ...
                # try if concatenating source sentences together improves bleu
            ...
            if targetgap and self.options['Nto1'] > 1:
                ...
                # try if concatenating target sentences together improves bleu
                # score (beginning of gap)
                if pregapsrc:
            ...
            # concatenation didn't help, and we still have possible one-to-one
            # alignments
    ...
    def gale_church(self, tempsrcgap, temptargetgap):
        ...
        pairs = sorted(
            align_texts(srclengths, targetlengths)[0], key=itemgetter(0))

        idict = {}
        jdict = {}
        newpairs = []

        # store 1-to-n alignments in single pairs of tuples (instead of using
        # multiple pairs of ints)
        for i, j in pairs:
                        ...
                        for iold2, jold2 in newpairs:
                            ...
                            if j in jold2:
                                if not (iold1, jold1) == (iold2, jold2):
                                    del(newpairs[
                                        newpairs.index((iold1, jold1))])
                                    del(newpairs[
                                        newpairs.index((iold2, jold2))])
                                    inew = tuple(
                                        sorted(list(iold1) + list(iold2)))
                                    jnew = tuple(
                                        sorted(list(jold1) + list(jold2)))
    ...
    # get a list of (ID,Sentence) tuples and generate bi- or tri-sentence
    # tuples
    ...
    def print_alignment_statistics(self, source_len, target_len):
        ...
        if self.options['verbosity'] >= 2:
            ...
            for sourceid in xrange(source_len):
        ...
        if source_len and target_len:
            self.log("\n" + str(len(self.bleualign)) + ' out of ' + str(source_len) +
                     ' source sentences aligned by BLEU ' + str(100 * len(self.bleualign) / float(source_len)) + '%', 2)
            self.log("after gap filling, " + str(multialignsrccount) + ' out of ' + str(source_len) +
                     ' source sentences aligned ' + str(100 * multialignsrccount / float(source_len)) + '%', 2)
            self.log("after gap filling, " + str(multialigntargetcount) + ' out of ' + str(target_len) +
                     ' target sentences aligned ' + str(100 * multialigntargetcount / float(source_len)) + '%', 2)
    ...
    def printout(self, sourcelist, translist, targetlist):
        ...
        for j, (src, target) in enumerate(i[0] for i in self.multialign):
            ...
            if self.options['printempty']:
                if src[0] != lastsrc + 1:
                    sources.extend(sourcelist[ID]
                                   for ID in xrange(lastsrc + 1, src[0]))
                    targets.extend('' for ID in xrange(lastsrc + 1, src[0]))
                    translations.extend(
                        '' for ID in xrange(lastsrc + 1, src[0]))

                if target[0] != lasttarget + 1:
                    sources.extend(
                        '' for ID in xrange(lasttarget + 1, target[0]))
                    targets.extend(
                        targetlist[ID] for ID in xrange(lasttarget + 1, target[0]))
                    translations.extend(
                        '' for ID in xrange(lasttarget + 1, target[0]))
            ...
            if self.options['factored']:
                ...
                sources_factored.append(
                    ' '.join(sourcelist[ID][1] for ID in src))
                targets_factored.append(
                    ' '.join(targetlist[ID][1] for ID in target))
        ...
        if self.options['filter'] == 'sentences':
            self.filter_sentence_pairs(
                sentscores, sources_output, targets_output)

        if self.options['filter'] == 'articles':
            self.filter_article_pairs(
        ...
        if self.out1 and self.out2 and not self.options['filter']:
            if self.options['factored']:
                self.out1.writelines(
                    [line + '\n' for line in sources_factored])
                self.out2.writelines(
    ...
    def check_sentence_pair(self, j, src, trans, target, source_out, target_out, sentscores):
        ...
        if sentscore2 > sentscore and self.options['filterlang']:
            ...
            if sentscore > 0:
                ...
                newsentscore = (2 * sentscore * sentscorex) / \
                    (sentscore + sentscorex)
            else:
                newsentscore = 0
    ...
    def score_article(self, test, ref):
        refs = [
            bleu.cook_refs([refSent], self.options['bleu_ngrams']) for refSent in ref]
        testcook = []

        for i, line in enumerate(test):
            testcook.append(
                bleu.cook_test(line, refs[i], self.options['bleu_ngrams']))
        ...
        for j, (src, target) in enumerate(i[0] for i in self.multialign):
            ...
            if j in sentscores:
                ...
                self.finalbleu.append(
                    (confidence, sentscores.get(j), before, before + 1))
                before += 1
                self.sources_out.append(sources_output[j])
                self.targets_out.append(targets_output[j])

    # store BLEU score for each article pair (used for filtering at the very
    # end)
    def filter_article_pairs(self, sources, translations, targets, sources_output, targets_output):
        ...
        self.log('\nBLEU score for article: ' +
                 str(articlescore) + ' / ' + str(articlescore2), 1)
        ...
        if articlescore2 > articlescore and self.options['filterlang']:
            ...
            if articlescore > 0:
                articlescore = (
                    articlescore * articlescorex * 2) / (articlescore + articlescorex)
            before = len(self.sources_out)
