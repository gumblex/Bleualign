...
def usage():
    ...
    print('\t' + bold + '--source' + reset +
          ', ' + bold + '-s' + reset + ' file')
    print('\t\tSource language text.')
    print('\t' + bold + '--target' + reset +
          ', ' + bold + '-t' + reset + ' file')
    print('\t\tTarget language text.')
    print('\t' + bold + '--output' + reset +
          ', ' + bold + '-o' + reset + ' filename')
    print('\t\tOutput file: Will create ' +
          'filename' + '-s and ' + 'filename' + '-t')
    ...
    print(
        '\t\tAlso write unaligned sentences to file. By default, they are discarded.')
    print('\t' + bold + '--verbosity' + reset +
          ', ' + bold + '-v' + reset + ' int')
    print(
...
def load_arguments(sysargv):
    ...
    for o, a in opts:
        if o in ("-h", "--help"):
            ...
            options['srcfile'] = os.path.join(
                project_path, 'eval', 'eval1989.de')
            options['targetfile'] = os.path.join(
                project_path, 'eval', 'eval1989.fr')
            ...
            options['srcfile'] = os.path.join(
                project_path, 'eval', 'eval1957.de')
            options['targetfile'] = os.path.join(
                project_path, 'eval', 'eval1957.fr')
            ...
            if a in ['sentences', 'articles']:
                ...
                print('\nERROR: Valid values for option ' + bold + '--filter' + reset +
                      ' are ' + bold + 'sentences ' + reset + 'and ' + bold + 'articles' + reset + '.')
                usage()
                sys.exit(2)
    ...
    if not options['output']:
        print(
            'WARNING: Output not specified. Just printing debugging output.', 0)
